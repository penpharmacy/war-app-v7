...
function distributeDose(total, option, skipDays) {
  ...
  const allowed = [];
  if (option === "both") allowed.push(2, 3);
  else allowed.push(parseInt(option));
  ...
  for (let days = 7; days >= 1; days--) {
    for (let d of allowed) {
      const totalTry = d * days;
      const diff = Math.abs(total - totalTry);
      if (diff < bestDiff) {
        bestDiff = diff;
        bestPlan = { dose: d, days: days };
      }
    }
  }

  if (bestPlan) {
    for (let i = 0; i < bestPlan.days && i < availableDays.length; i++) {
      daily[availableDays[i]] = bestPlan.dose;
    }
  }

  // ปรับวันสุดท้ายถ้ายังขาดอยู่และใช้ได้หลายขนาด
  const currentSum = daily.reduce((a, b) => a + b, 0);
  const remain = total - currentSum;
  if (option === "both" && remain > 0.1) {
    for (let d of allowed) {
      if (remain === d) {
        daily[bestPlan.days] = d;
        break;
      }
    }
  }
